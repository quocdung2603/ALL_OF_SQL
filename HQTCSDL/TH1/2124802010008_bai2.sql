...
--------------------------- CAU 7---------------------
--a
create procedure ThemNhanVien (@manv int output, @tennv nvarchar(50) , @tuoi tinyint , @gioitinh bit )
as
begin
	set @manv = (select max(manv) from Nhanvien)
	insert into Nhanvien (manv , tennv, tuoi , gioitinh) values (@manv+1 , @tennv, @tuoi, @gioitinh)
end
go 
--execute Themnhanvien  '5', @tennv='Nguyen Quoc Dung'  , @tuoi=19 , @gioitinh=0
go
--b
...
END
GO
--exec SuaMaNV @manv= 7, @manv_new=5
--drop proc SuaMaNV
